...
def compute_from_patches(wsi_object, clam_pred=None, model=None, feature_extractor=None, batch_size=512,  
    ...
    roi_loader = get_simple_loader(roi_dataset, batch_size=batch_size, num_workers=8) #change num_workers from 8 to 0 if on a windows machine
            ...
            if attn_save_path is not None:
                ...
                if ref_scores is not None:
                    ...
                    for score_idx in range(len(A)):
                        A[score_idx] = score2percentile(A[score_idx], ref_scores_flat)
