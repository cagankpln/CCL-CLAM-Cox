...
def c_index_lifelines(risks, events, times):
    ...
    print("Filtered risks:", new_risk)
    print("Filtered times:", new_surv)
    print("Filtered events:", new_label)

    # Check if there are any admissible pairs before calling concordance_index
    if len(new_risk) == 0 or len(new_label) == 0 or len(new_surv) == 0:
        raise ValueError("Filtered data contains no valid entries for c-index calculation.")

    print("Calling concordance_index with:")
    print("new_surv:", new_surv)
    print("new_risk:", new_risk)
...
def neg_partial_loglik(preds, events, times):
    ...
    risk_set = torch.FloatTensor(risk_set).to(device)
    events = torch.FloatTensor(events).to(device)
