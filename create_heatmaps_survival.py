...
if __name__ == '__main__':
	...
	if model_args.premodel_type == "retccl":
		feature_extractor = ResNet.resnet50(num_classes=128, mlp=False, two_branch=False, normlinear=True)
		pretext_extractor = torch.load('best_ckpt.pth', map_location="cuda" if torch.cuda.is_available() else "cpu")
		feature_extractor.fc = nn.Identity()
		feature_extractor.load_state_dict(pretext_extractor, strict=True)
		feature_extractor.eval()
		device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
		print("Done!")
